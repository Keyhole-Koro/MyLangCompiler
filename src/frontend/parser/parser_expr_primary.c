...
static ASTNode *parse_case_primary(ParserContext *context, Token **cur) {
    *cur = (*cur)->next;
    ASTNode *target = parse_expr(context, cur);
    if (!expect(cur, OF)) parse_error(context, "expected 'of' after case target", *cur);
    if (!expect(cur, L_BRACE)) parse_error(context, "expected '{' after of", *cur);
...
static ASTNode *parse_identifier_primary(ParserContext *context, Token **cur) {
    ...
    *cur = (*cur)->next;
    ...
    ASTNode *generic_declaration = find_generic_function_template(context, name);
    int is_generic_function = generic_declaration != NULL;
    if ((*cur)->kind == LT &&
        (is_generic_function ||
         (context->control.current_generic_function_name && strcmp(context->control.current_generic_function_name, name) == 0))) {
        type_args = parse_type_args(context, cur, &type_arg_count);
        if ((*cur)->kind != L_PARENTHESES)
            parse_error(context, "expected '(' after generic function arguments", *cur);
    ...
    long enum_val;
    if (find_enum_constant(context, name, &enum_val)) {
        char buf[32];
...
ASTNode *parse_primary(ParserContext *context, Token **cur) {
    ...
    if ((*cur)->kind == L_PARENTHESES && looks_like_fun_literal(context, *cur)) {
        *cur = (*cur)->next;
        ASTNode **params = NULL;
        int param_count = 0;
        ...
        if (!expect(cur, R_PARENTHESES)) parse_error(context, "expected ')' after function literal parameters", *cur);
    ...
    if ((*cur)->kind == L_PARENTHESES) {
        *cur = (*cur)->next;
        if ((*cur)->kind == L_BRACE) {
            ASTNode *block = parse_block(context, cur);
            if (!expect(cur, R_PARENTHESES)) parse_error(context, "expected ')' after statement expression", *cur);
            return new_stmt_expr(block);
        }
        ASTNode *node = parse_expr(context, cur);
        if (!expect(cur, R_PARENTHESES)) parse_error(context, "expected ')'", *cur);
        return node;
    }
    parse_error(context, "expected primary", *cur);
    return NULL;
}
