...
char **parse_type_params(Token **cur, int *out_count, int add_to_scope) {
    ...
    if (!expect(cur, LT)) parse_error("expected '<' before type parameters", token_head, *cur);
    if ((*cur)->kind == GT) parse_error("generic declaration requires at least one type parameter", token_head, *cur);

    while (1) {
        if ((*cur)->kind != IDENTIFIER)
            parse_error("expected type parameter name", token_head, *cur);
        if (contains_name(params, count, (*cur)->value))
            parse_error("duplicate type parameter", token_head, *cur);
        ...
        *cur = (*cur)->next;
        if ((*cur)->kind == GT)
            parse_error("trailing comma in type parameter list", token_head, *cur);
    }

    if (!expect(cur, GT)) parse_error("expected '>' after type parameters", token_head, *cur);
    *out_count = count;
    return params;
}
...
ASTNode **parse_type_args(Token **cur, int *out_count) {
    ...
    if (!expect(cur, LT)) parse_error("expected '<' before type arguments", token_head, *cur);
    if ((*cur)->kind == GT) parse_error("generic use requires at least one type argument", token_head, *cur);
    ...
    while (1) {
        ...
        *cur = (*cur)->next;
        if ((*cur)->kind == GT)
            parse_error("trailing comma in type argument list", token_head, *cur);
    }

    if (!expect_type_arg_close(cur)) parse_error("expected '>' after type arguments", token_head, *cur);
    *out_count = count;
    return args;
}
...
ASTNode *parse_generic_fundef(Token **cur) {
    ...
    if (!params_start) parse_error("invalid generic function declaration", token_head, *cur);

    Token *name_tok = *cur;
    while (name_tok && name_tok->next != params_start) name_tok = name_tok->next;
    if (!name_tok || name_tok->kind != IDENTIFIER)
        parse_error("expected generic function name", token_head, *cur);
    ...
    const char *previous_generic_function = g_current_generic_function_name;
    g_current_generic_function_name = name_tok->value;
    g_generic_decl_depth++;
    ASTNode *fn = parse_fundef(cur);
    g_generic_decl_depth--;
    g_current_generic_function_name = previous_generic_function;
